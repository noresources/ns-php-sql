...
trait TransactionStackTrait
{
	...
	public function newTransactionBlock($name = null)
	{
		...
		{
			if ($block->getBlockState() != TransactionBlockInterface::STATE_PENDING)
			{
		...
		if (!\is_callable($this->blockFactory))
			throw new \RuntimeException('Transaction block factory was not defined.');
	...
	/**
	 * This method should have to ve called on disconnection or destruction of the ConnectionInterface.
	 *
	...
	private function endTransactions($commit)
	{
		...
		{
			if ($this->transactionStack->getBlockState() == TransactionBlockInterface::STATE_PENDING)
			{
				if ($commit)
