	...
	ConnectionProviderInterface
{
	...
	public function getTablePrimaryKeyConstraint($tableIdentifier)
	{
		...
		$autoGenerated = $this->getAutogeneratedConstraintName($pk,
			$tableIdentifier);
		if ($autoGenerated == $pk->getName())
			$pk->setName(null);
		return $pk;
	}
	...
	public function getTableForeignKeyConstraints($tableIdentifier)
	{
		...
		foreach ($names as $name)
		{
			...
			$autoGenerated = $this->getAutogeneratedConstraintName($fk,
				$tableIdentifier);
...
', $tableOID, $platform->quoteStringValue('f'));
	...
	public function getAutogeneratedConstraintName(
		TableConstraintInterface $constraint, $parent = null)
	{
		$parts = [];
		if ($parent)
			$parts[] = Identifier::make($parent)->getLocalName();
		elseif ($constraint->getParentElement())
			$parts[] = $constraint->getParentElement()->getName();
		if ($constraint instanceof ForeignKeyTableConstraint)
		{
			$parts[] = Container::firstKey($constraint->getColumns());
			$parts[] = 'fkey';
		}
		elseif ($constraint instanceof PrimaryKeyTableConstraint)
		{
			$parts[] = 'pkey';
		}
		elseif ($constraint instanceof CheckTableConstraint)
		{
			$parts[] = 'check';
		}
		elseif ($constraint instanceof KeyTableConstraintInterface)
		{
			$parts[] = Container::firstValue($constraint->getColumns());
			$parts[] = 'key';
		}
