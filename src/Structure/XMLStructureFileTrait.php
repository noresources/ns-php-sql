...
trait XMLStructureFileTrait
{
	...
	public static function getXmlNamespaceURI(SemanticVersion $schemaVersion)
	{
		...
		if ($schemaVersion->getIntegerValue() > 10099)
			$s .= '/' . $schemaVersion->slice(SemanticVersion::MAJOR, SemanticVersion::MINOR);
		return $s;
	}
	...
	public static function getDataTypeFromNodeName($nodeName, SemanticVersion $schemaVersion)
	{
	...
	public static function getDataTypeNode($dataType, SemanticVersion $schemaVersion)
	{
		if (!Container::isArray(self::$dataTypeNodeNames))
			self::initializeDataTypeNodeNames();

		return Container::keyValue(self::$dataTypeNodeNames, $dataType, 'string');
	}
	 ...
	 */
	public static function getDefaultNodeValueNodeName($columntDataType, $defaultValueType,
